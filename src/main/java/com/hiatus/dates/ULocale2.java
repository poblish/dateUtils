...
public class ULocale2
{
	...
	*******************************************************************************/
	...
	/*******************************************************************************
	*******************************************************************************/
	public static DateFormat getClientDateTimeFormat( Locale inLocale)
	{
		...
		try
		{
			...
			else	theFormat = DateFormat.getDateTimeInstance();
		}
		catch (Exception e)
		{
	...
	/*******************************************************************************
	*******************************************************************************/
	public static DateFormat getClientDateTimeFormat( Locale inLocale, int inTimeFormat)
	{
		DateFormat	theFormat;
	...
	/*******************************************************************************
	*******************************************************************************/
	public static Locale getCorrectedLocale( Locale inLocale)
	{
		String	theLang = inLocale.getLanguage();	// Under some circumstances this could be wrong, eg "en_gb" instead of "en"
								// The "en_gb" problem was noticed using OmniWeb 4 on Mac OS X - May 2001

		if (( theLang.length() >= 5) && ( inLocale.getCountry().length() < 1))
		{
			int	theUScorePos = theLang.indexOf('_');	// eg. "en_gb"
	...
	/*******************************************************************************
		29 May 2002
	*******************************************************************************/
	public static String getLocaleHttpHeaderString( Locale inLocale)
	{
		if ( inLocale != null)
		{
			String	theLang = inLocale.getLanguage();

			if (!theLang.isEmpty())
			{
				String	theCountry = inLocale.getCountry();
	...
	/*******************************************************************************
	*******************************************************************************/
	public static String myGetDisplayLanguage( Locale inLocale)
	{
		Locale	theLoc = getCorrectedLocale(inLocale);
		return theLoc.getDisplayLanguage(theLoc);
	}

	/*******************************************************************************
	*******************************************************************************/
	public static Locale stringToLocale( String inString)
	{
		if (( inString != null) && ( inString.length() >= 2))
		{
			try
			{
				String	theLang = inString.substring( 0, 2);
				int	thePos = inString.indexOf("-");
				String	theCountry = ( thePos >= 0) ? inString.substring( thePos + 1) : "";
	...
	*******************************************************************************/
	public static String getBestTimeZoneStr( Locale inLocale)
	{
		String	theLang = inLocale.getLanguage();
		String	theCountry = inLocale.getCountry();
...
//		println("lang \"" + theLang + "\", country \"" + inCountry + "\", theID = \"" + theID + "\".");
	...
	/*******************************************************************************
		20 May 2002
	*******************************************************************************/
	public static String getCapitalCityString( Locale inLocale)
	{
		String	theZoneStr = getBestTimeZoneString( inLocale.getLanguage(), inLocale.getCountry());
		int	theDashPos = theZoneStr.indexOf('/');
	...
	/*******************************************************************************
		should return GMT if no match
	*******************************************************************************/
	public static TimeZone getBestTimeZone( Locale inLocale)
	{
	...
	/*******************************************************************************
		8 April 2002
	*******************************************************************************/
	public static GregorianCalendar getGregorianCalendar( Locale inLocale)
	{
	...
	/*******************************************************************************
	*******************************************************************************
	public static void PrintOutLocales()
	{
		Locale		theDefaultLocale = Locale.getDefault();
		Locale[]	theLocalesList = LocaleData.getAvailableLocales("Countries");

		for ( int i = 0; i < theLocalesList.length; i++)
		{
			info( i + " ... " + theLocalesList[i].getDisplayCountry(theDefaultLocale) + ", " +
						theLocalesList[i].getDisplayLanguage(theDefaultLocale) +
						" (" + theLocalesList[i] + ")");
		}
	}/
	...
	/***********************************************************************************
		2 August 2001
	***********************************************************************************/
	public static int getLocaleDateOrder( Locale inLocale)
	{
		SimpleDateFormat	theFmt = (SimpleDateFormat) DateFormat.getDateInstance( DateFormat.SHORT, inLocale);
		String			thePattern = theFmt.toPattern().toLowerCase(Locale.UK);		// simple Lowercase
		int			theLoc_D = thePattern.indexOf('d');
		int			theLoc_M = thePattern.indexOf('m');
		int			theLoc_Y = thePattern.indexOf('y');

		if (( theLoc_D < theLoc_Y) && ( theLoc_Y < theLoc_M))		return ORDER_DYM;
		else if (( theLoc_D < theLoc_M) && ( theLoc_M < theLoc_Y))	return ORDER_DMY;

		else if (( theLoc_Y < theLoc_D) && ( theLoc_D < theLoc_M))	return ORDER_YDM;
		else if (( theLoc_Y < theLoc_M) && ( theLoc_M < theLoc_D))	return ORDER_YMD;

		else if (( theLoc_M < theLoc_D) && ( theLoc_D < theLoc_Y))	return ORDER_MDY;
		else if (( theLoc_M < theLoc_Y) && ( theLoc_Y < theLoc_D))	return ORDER_MYD;

		else	return ORDER_DYM;
	}

	/*******************************************************************************
	*******************************************************************************/
	public static String getBestTimeZoneString( String inLang, String inCountry)
	{
		String	theZoneStr;

		if ( inCountry.length() > 0)			// 8 August 2001 !
		{
			theZoneStr = (String) s_OverridesMap.get(inCountry);
			if ( theZoneStr != null)
			{
				return theZoneStr;
			}
		...
		theZoneStr = (String) s_LangDefaultsMap.get(inLang);
	...
	/*******************************************************************************
		2 August 2001
	*******************************************************************************/
	public static void joinDateEntryParts( Locale inLocale, StringBuffer ioBuf,
						StringBuffer inDBuf, StringBuffer inMBuf, StringBuffer inYBuf)
	{
		int	theOrder = getLocaleDateOrder(inLocale);

		if ( theOrder == ORDER_DMY)		ioBuf.append(inDBuf).append(inMBuf).append(inYBuf);
		else if ( theOrder == ORDER_DYM)	ioBuf.append(inDBuf).append(inYBuf).append(inMBuf);
		else if ( theOrder == ORDER_MDY)	ioBuf.append(inMBuf).append(inDBuf).append(inYBuf);
		else if ( theOrder == ORDER_MYD)	ioBuf.append(inMBuf).append(inYBuf).append(inDBuf);
		else if ( theOrder == ORDER_YDM)	ioBuf.append(inYBuf).append(inDBuf).append(inMBuf);
		else if ( theOrder == ORDER_YMD)	ioBuf.append(inYBuf).append(inMBuf).append(inDBuf);
	}

	/*******************************************************************************
		Note: From 1st November 2000 we display and treat all money amounts
		in US$, to save ourselves the hassle of converting currencies.
	*******************************************************************************/
